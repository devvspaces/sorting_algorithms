...
/**
 * _deck_swap - swaps 2 consecutive cards of a doubly linked list
 ...
 */
void _deck_swap(deck_node_t **deck, deck_node_t *card1, deck_node_t *card2)
{
...
/**
 * _card_cmp - compares 2 card values
 *
 ...
 */
int _card_cmp(const card_t *card1, const card_t *card2)
{
...
/**
 * _sort_deck_value - sorts a 52 card deck by value
 *
 * @deck: deck to sort
 */
void _sort_deck_value(deck_node_t **deck)
{
	deck_node_t *f, *tmp;

	for (f = (*deck)->next; f && f->prev; f = f->next)
	{
		for (; f && f->prev && _card_cmp(f->card, f->prev->card) &&
			   f->card->kind == f->prev->card->kind;
			 f = f->prev)
		{
			tmp = f->prev;
			_deck_swap(deck, tmp, f);
			f = f->next;
		}
	}
}
